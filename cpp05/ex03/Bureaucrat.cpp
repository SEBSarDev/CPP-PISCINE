...
void Bureaucrat::executeForm(Form const &form)
{
	try
	{
		form.checkError(*this);
		form.execute(*this);
		std::cout << GREEN << this->_name << " executs " << form.getName() << RESET << std::endl;
	}
	catch (std::exception &e)
	{
		std::cout << RED << this->_name << " cannot execute because " << e.what() << RESET << std::endl;
	}
}
void Bureaucrat::signForm(Form &form)
{
	...
	{
		std::cout << RED << this->_name << " cannot sign because " << e.what() << RESET << std::endl;
	}
}
